...
df = pd.read_csv('data/Belgium.csv', sep=',')
...
start_date = T('2020/3/21')  # T=0
end_date = T('2020/4/17')  #09/02
...
alpha_list = np.arange(1.0,  1.11,  step=0.1) * 1e-08
beta_list = np.arange(1.0,  1.51,  step=0.1) * 1e-09

num_I_list = np.arange(3600,4201, step=300)
I2E_factors = np.arange(1.0, 1.51, step=0.1)
I2M_factors = np.arange(0.5, 1.01, step=0.1)

k_days_list = [14]
x0_pt_list =[12955]

'''
alpha_list = np.arange(0.8,  1.81,  step=0.5) * 1e-08
beta_list = np.arange(0.8,  2.31,  step=0.5) * 1e-09

num_I_list = np.arange(2000,10000, step=5000)
I2E_factors = np.arange(0.6, 1.81, step=0.5)
I2M_factors = np.arange(0.2, 1.21, step=0.5)

k_days_list = [14]
x0_pt_list = np.arange(0, 30000, step=10000)
...
def one_run(initial_num_I, I2E_factor, I2M_factor, alpha, beta, mu_ei, mu_mo, k_days, x0_pt):
    ...
    for i in range((T('2020/4/17') - start_date).days, total.shape[0]-1):
