...
from const2 import STATE, COLORS, NUM_STATES,  STATES



df = pd.read_csv('data/Czechia.csv', sep=',')
...
start_date = T('2020/3/25')  # T=0
end_date = T('2020/5/5')  #09/02
...
'''
alpha_list = np.arange(1.0,  1.21,  step=0.1) * 1e-08
beta_list = np.arange(2.0,  2.51,  step=0.1) * 1e-09

num_I_list = np.arange(1000,2001, step=100)
I2E_factors = np.arange(0.5, 1.01, step=0.1)
I2M_factors = np.arange(0.1, 0.71, step=0.1)
'''


alpha_list = np.arange(1.1,  1.11,  step=0.1) * 1e-08
beta_list = np.arange(2.2,  2.21,  step=0.1) * 1e-09

num_I_list = np.arange(1400,1401, step=100)
I2E_factors = np.arange(0.9, 0.91, step=0.1)
I2M_factors = np.arange(0.1, 0.11, step=0.1)
...
def one_run(initial_num_I, I2E_factor, I2M_factor, alpha, beta, mu_ei, mu_mo, k_days, x0_pt):
    ...
    for i in range((T('2020/5/7') - start_date).days, total.shape[0]-1):
