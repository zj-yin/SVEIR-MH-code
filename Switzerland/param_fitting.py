...
df = pd.read_csv('data/Switzerland.csv', sep=',')
df['date'] = df['date'].apply(T)


### ATTENTION
# start and end date of date being  fitted
start_date = T('2020/3/20')  # T=0
end_date = T('2020/5/12')  #09/02
...
alpha_list = np.arange(1.3,  1.51,  step=0.1) * 1e-08
beta_list = np.arange(1.3,  1.51,  step=0.1) * 1e-09
...
'''
num_I_list = np.arange(36000,90001, step=3000)
I2E_factors = np.arange(0.8, 2, step=0.2)
I2M_factors = np.arange(0.2, 1.6, step=0.2)


k_days_list = np.arange(11, 21, 3)
x0_pt_list = np.arange(20000, 80001, 20000)
'''
'''
num_I_list = np.arange(6000,24001, step=3000)
I2E_factors = np.arange(0.6, 1.61, step=0.3)
I2M_factors = np.arange(0.6,0.81, step=0.2)
'''
num_I_list =[7200]
I2E_factors = np.arange(0.5, 0.71, step=0.1)
I2M_factors=np.arange(0.5, 0.71, step=0.1)
...
def one_run(initial_num_I, I2E_factor, I2M_factor, alpha, beta, mu_ei, mu_mo, k_days, x0_pt):
    ...
    total, delta, increase, trans_data, ax = do_simulation(
        total_days+1, bed_info, params, p0_time=start_date
    )
    ...
    for i in range((T('2020/5/12') - start_date).days, total.shape[0]-1):
    ...
    row = (
        ...
        I_mae, is_decreasing_after_Feb09
    )
    return row
...
print('=' * 10)
print(br)
makedir_if_not_there('output')
br.to_csv('output/best parameter.csv', index=1)   ###写出最优参数
