...
start_date = T('2020/3/22')  # T=0
end_date = T('2020/4/13')  #09/02
...
beta_list = np.arange(1.4,  1.81,  step=0.1) * 1e-09
...
num_I_list = np.arange(3600,5401, step=360)
I2E_factors =np.arange(1.4, 1.81, step=0.2)
I2M_factors = np.arange(0.6, 1.21, step=0.1)

k_days_list =[14]
#x0_pt_list = np.arange(0, 36001, step=3000)
x0_pt_list =[18000]
mu_ei_list = [6]
mu_mo_list = [10]
...
def one_run(initial_num_I, I2E_factor, I2M_factor, alpha, beta, mu_ei, mu_mo, k_days, x0_pt):
    ...
    for i in range((T('2020/4/13') - start_date).days, total.shape[0]-1):
