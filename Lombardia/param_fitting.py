...
df = pd.read_csv('data/Lombardia.csv', sep=',')
...
start_date = T('2020/3/15')  # T=0
end_date = T('2020/4/30')  #09/02
...
alpha_list = np.arange(1.2,  1.41,  step=0.1) * 1e-08
beta_list = np.arange(1.3,  1.51,  step=0.1) * 1e-09

num_I_list = np.arange(6000,24001, step=2000)
I2E_factors = np.arange(0.6, 1.6, step=0.2)
I2M_factors = np.arange(0.6, 1.6, step=0.2)
...
'''
alpha_list = np.arange(0.2,  2,  step=0.6) * 1e-08
beta_list = np.arange(0.2,  2,  step=0.6) * 1e-09

num_I_list = np.arange(12000,60001, step=30000)
I2E_factors = np.arange(0.6, 1.8, step=0.6)
I2M_factors = np.arange(0.2, 1.2, step=0.5)



k_days_list = np.arange(8, 18, 7)
x0_pt_list = np.arange(0, 15001, step=8000)
'''
mu_ei_list = [6]
mu_mo_list = [10]
...
def one_run(initial_num_I, I2E_factor, I2M_factor, alpha, beta, mu_ei, mu_mo, k_days, x0_pt):
    ...
    for i in range((T('2020/4/30') - start_date).days, total.shape[0]-1):
...
br.to_csv('output/best parameter.csv', index=None)   ###写出最优参数
