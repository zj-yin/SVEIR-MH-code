...
df = pd.read_csv('data/Sweden_2.csv', sep=',')
...
start_date = T('2020/6/11')  # T=0
end_date = T('2020/8/2')  #09/02
...
alpha_list = np.arange(1.2,  1.21,  step=0.1) * 1e-08
beta_list = np.arange(1.6,  1.61,  step=0.1) * 1e-09

num_I_list = np.arange(6400,6401, step=400)
I2E_factors = np.arange(0.9, 0.91, step=0.2)
I2M_factors = np.arange(1.3, 3.31, step=0.2)
...
def one_run(initial_num_I, I2E_factor, I2M_factor, alpha, beta, mu_ei, mu_mo, k_days, x0_pt):
    ...
    for i in range((T('2020/8/4') - start_date).days, total.shape[0]-1):
