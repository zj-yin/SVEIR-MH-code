...
params_before = pkl.load(open('output/params_before_lockdown_First.pkl', 'rb'))
...
lockdown_time =T('2020/3/7')
days_before_ld  = (lockdown_time -  p0_time).days
fine_grained_alpha = [(0, params_before.alpha), (days_before_ld, params_before.alpha/1)]
fine_grained_beta = [(0, params_before.beta), (days_before_ld, params_before.beta/1)]
for i in days_offsets:
    if (days_before_ld + i)<=32:
        fine_grained_alpha += [(days_before_ld + i, params_before.alpha/1)]
        fine_grained_beta += [(days_before_ld + i, params_before.beta/1)]
    else:
        if (days_before_ld + i)<=37:
            fine_grained_alpha += [(days_before_ld + i, params_before.alpha/2.25)]
            fine_grained_beta += [(days_before_ld + i, params_before.beta/2.25)]
        else:
            if (days_before_ld + i)<=57:
                fine_grained_alpha += [(days_before_ld + i, params_before.alpha/1.25)]
                fine_grained_beta += [(days_before_ld + i, params_before.beta/1.25)]
            else:
                if (days_before_ld + i)<=123:
                    fine_grained_alpha += [(days_before_ld + i, params_before.alpha/2)]
                    fine_grained_beta += [(days_before_ld + i, params_before.beta/2)]
                else:
                    if (days_before_ld + i)<=240:
