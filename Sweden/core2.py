...
from helpers2 import  pr_EI_long, pr_MO_long, pr_IM_long, get_T1_and_T2, R0
from const2 import STATE, STATES, NUM_STATES, COLORS, TRANS,NUM_TRANS
...
def do_simulation(
    ...
    data[0, STATE.V1] = params.initial_num_V1
    data[0, STATE.V2] = params.initial_num_V2
    data[0, STATE.EV1] = params.initial_num_EV1
    ...
    delta_data[0, STATE.V1] = params.initial_num_V1
    delta_data[0, STATE.V2] = params.initial_num_V2
    delta_data[0, STATE.EV1] = params.initial_num_EV1
    ...
    increase_data[0, STATE.V1] = params.initial_num_V1
    increase_data[0, STATE.V2] = params.initial_num_V2
    increase_data[0, STATE.EV1] = params.initial_num_EV1
    ...
    trans_data[0, TRANS.S2V1] = 0
    trans_data[0, TRANS.S2V2] = 0
    trans_data[0, TRANS.V12EV1] = 0
    trans_data[0, TRANS.E2I] = 0
    trans_data[0, TRANS.I2M] = 0
    trans_data[0, TRANS.I2O] = 0
    trans_data[0, TRANS.EV12O] = 0
    #trans_data[0, TRANS.V22O] = 0
    trans_data[0, TRANS.M2O] = 0
    trans_data[0, TRANS.EbyE] = 0
    trans_data[0, TRANS.EbyI] = 0
    trans_data[0, TRANS.EbyEV1] = 0
    trans_data[0, TRANS.EV1byE] = 0
    trans_data[0, TRANS.EV1byI] = 0
    trans_data[0, TRANS.EV1byEV1] = 0
    ...
    num_in_EV1 = np.zeros((total_days+1), dtype=float)
    num_in_EV1[0] = params.initial_num_EV1
    ...
    for T in iters:
        ...
        inf_proba_EV1 = min(1, data[T-1, STATE.EV1] * 0.5*params.alpha_func(T-1))
        ...
        # infection by E or I or EV1
        inf_proba_sum = inf_proba_E + inf_proba_I+inf_proba_EV1
        if inf_proba_sum > 1:
            # bound it from above by 1
            inf_proba_E /= inf_proba_sum
            inf_proba_I /= inf_proba_sum

        inf_proba = inf_proba_E + inf_proba_I+inf_proba_EV1
        ...
        '''
        E_by_EV1 = 0.5*inf_proba_EV1 * data[T-1, STATE.S]
        EV1_by_E = 0.5*inf_proba_E * data[T-1, STATE.V1]
        EV1_by_I = 0.5*inf_proba_I * data[T-1, STATE.V1]
        EV1_by_EV1 = 0.25*inf_proba_EV1 * data[T-1, STATE.V1]
        '''
        
        
        E_by_EV1 = inf_proba_EV1 * data[T-1, STATE.S]
        EV1_by_E = inf_proba_E * data[T-1, STATE.V1]
        EV1_by_I = inf_proba_I * data[T-1, STATE.V1]
        EV1_by_EV1 = inf_proba_EV1 * data[T-1, STATE.V1]
		
        
        day_offsets = [t for t in range(1, params.k_days+1) if T - t >= 0]

        S2E = (data[T-1, STATE.S] * inf_proba)
        V12EV1=(data[T-1, STATE.V1] * inf_proba)
        ...
        '''
        T=42 means begin of vaccine is 42-14=28
        '''
        '''
    
        if (T>=374) & (T<=720)& (data[T-1, [STATE.S]]>50000):
            S2V1 =4500
            S2V2 =5000
        else:
            S2V1 =0
    ...
    '''
        ...
        #EV12O=data[T-14,V12EV1]
        
        EV12O=(data[T-14, STATE.V1] * inf_proba)
        
        if EV12O>data[T-1, STATE.EV1]+V12EV1:
            EV12O=data[T-1, STATE.EV1]+V12EV1
        

        
        #V22O=data[T-1, STATE.V2]
        ...
        increase_data[T, STATE.EV1]=V12EV1
        increase_data[T, STATE.V1]=S2V1
        increase_data[T, STATE.V2]=S2V2
        ...
        increase_data[T, STATE.O] = M2O + I2O+EV12O
        ...
        for trans, v in zip(('S->E','S->V1','S->V2','V1->EV1','EV1->O','V2->O','E->I', 'I->O', 'I->M', 'M->O'), (S2E, S2V1,S2V2,V12EV1,EV12O,E2I, I2O, I2M, M2O)):
            if np.isclose(v, 0):
                v = 0
        ...
        for v in [S2E,S2V1,S2V2,V12EV1,EV12O,E2I,I2O,I2M,M2O]:
        ...
        delta_S = -S2E-S2V1-S2V2
        delta_E = S2E - E2I
        delta_V1 = S2V1-V12EV1
        delta_V2 = S2V2
        delta_EV1 = V12EV1 - EV12O
        delta_I = E2I - I2M - I2O
        delta_M = I2M - M2O
        delta_O = I2O + M2O+EV12O



        data[T, STATE.S] = data[T-1, STATE.S] + delta_S
        data[T, STATE.E] = data[T-1, STATE.E] + delta_E
        data[T, STATE.V1] = data[T-1, STATE.V1] + delta_V1
        data[T, STATE.V2] = data[T-1, STATE.V2] + delta_V2
        data[T, STATE.EV1] = data[T-1, STATE.EV1] + delta_EV1
        ...
        trans_data[T, TRANS.S2V1] = S2V1
        trans_data[T, TRANS.S2V2] = S2V2
        trans_data[T, TRANS.V12EV1] = V12EV1
        trans_data[T, TRANS.I2M] = I2M
        trans_data[T, TRANS.I2O] = I2O
        trans_data[T, TRANS.M2O] = M2O
        trans_data[T, TRANS.EV12O] = EV12O
        #trans_data[T, TRANS.V22O] = V22O
        trans_data[T, TRANS.EbyE] = E_by_E
        trans_data[T, TRANS.EbyI] = E_by_I
        trans_data[T, TRANS.EbyEV1] = E_by_EV1
        trans_data[T, TRANS.EV1byE] = EV1_by_E
        trans_data[T, TRANS.EV1byI] = EV1_by_I
        trans_data[T, TRANS.EV1byEV1] = EV1_by_EV1
        
        
        '''
        if trans_data[T-1, TRANS.EV12O] > data[T-1, [STATE.EV1]]:
            EV12O = 0
        '''
        ...
        delta_data[T, STATE.V1] = delta_V1
        delta_data[T, STATE.V2] = delta_V2
        delta_data[T, STATE.EV1] = delta_EV1
        ...
        '''
        if data[T-1, [STATE.S]]<90000:
            trans_data[T, TRANS.S2V1]=0
       
        if data[T-1, [STATE.S]]<90000:
            trans_data[T, TRANS.S2V2]=0
        '''
