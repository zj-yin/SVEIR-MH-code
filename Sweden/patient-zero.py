...
from helpers2 import Params, T, get_T1_and_T2, R0, plot_total, save_bundle, save_to_json, makedir_if_not_there
...
'''
infection_factor=np.float64(2.26)
...
p0_time = T('2020/6/10') - timedelta(days=best_t)
print(p0_time)




makedir_if_not_there('output_test/tbl/p0-time/')




df.to_csv('output_test/tbl/p0-time/error.csv', index=False)





pkl.dump(p0_time, open('output_test/p0_time.pkl', 'wb'))
...
makedir_if_not_there('output_test/tbl/before-lockdown/')




save_bundle([total, delta, increase, trans], p0_time, best_t, 'output_test/tbl/before-lockdown')
save_to_json(stats, 'output_test/tbl/before-lockdown/stat.txt')



pkl.dump(
    params,
    open('output_test/params_before_lockdown.pkl', 'wb')
)
